...
<div class="container">
    ...
    <h2>Mes notes</h2>
               <?php
			$notes = $db->getNoteEtudiant($_SESSION['mail']);
		?>  
  <table class="table">
    <thead>
      <tr>
    ...
    <tbody>
	<?php
	while($donnees = $notes->fetch())
        {
	?>
      <tr>
        <td><?php echo $donnees['libMatiere'];?></td>
        <td><?php echo $donnees['noteEleve'];?></td>
        <td><?php echo $donnees['noteMax'];?></td>
        <td><?php echo $donnees['titreDevoir'];?></td>
        <td><?php echo $donnees['professeur'];?></td>
        </tr>
        <?php
         echo $_SESSION['mail'];  } 
      ?>
